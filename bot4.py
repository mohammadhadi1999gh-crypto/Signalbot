...
PRESETS = {
    "conservative": dict(touches=1, fvg_atr=0.25, base_min=5, min_score=6, strict=True,  room=1.0, cooldown=6, spike=2.5, hunt=3),
    "balanced":     dict(touches=2, fvg_atr=0.20, base_min=4, min_score=5, strict=False, room=0.8, cooldown=4, spike=2.0, hunt=2),
    "aggressive":   dict(touches=3, fvg_atr=0.15, base_min=4, min_score=4, strict=False, room=0.6, cooldown=3, spike=1.7, hunt=2),
}
_SENS = os.getenv("SENSITIVITY", "aggressive").lower()
_PR = PRESETS.get(_SENS, PRESETS["aggressive"])


class CFG:
    sensitivity = _SENS if _SENS in PRESETS else "aggressive"
    ...
    workers = _f("WORKERS", 4)                                   # تعداد ارز که هم‌زمان اسکن می‌شود
...
def detect_spike(P, t, t1, t2, extra=()):
    ...
    if lw >= 0.5 * rng and (c - l) >= 0.5 * rng and lw >= 0.4 * a:
        ...
        return None
    if t1 == -side and t2 == -side:           # هر دو تایم‌فریم بالاتر قویاً مخالف
        return None
    rsi = P["rsi"][t]
    if (side == 1 and rsi > 75) or (side == -1 and rsi < 25):
        return None
...
def detect_hunter(P, t, t1, t2, extra=()):
    ...
    for side in order:
        if t1 * side < 0 or t2 * side < 0:
            continue
        if (side == 1 and rsi > 70) or (side == -1 and rsi < 30):
            continue
...
def detect_all(P, t, t1, t2, extra=()):
    """همه‌ی سیگنال‌های ممکن روی این کندل: اسپایک، FVG/ترندلاین، شکارچی"""
    out = []
    if CFG.spike_on:
        sg = detect_spike(P, t, t1, t2, extra)
        if sg:
            out.append(sg)
...
def kind_group(kind):
    return {"FVG": "SETUP", "TL": "SETUP", "SPIKE": "SPIKE", "HUNT": "HUNT"}.get(kind, "SETUP")
...
def make_chart(sym, tf, P, t, sig, info, breaks, bars=90):
    ...
    zone_lbl = {"FVG": f" FVG {'Bullish' if long_ else 'Bearish'}", "TL": " Trendline zone",
                "SPIKE": f" Spike wick x{sig.get('spike_x', 0):.1f} ATR", "HUNT": " Liquidity pool (limit ladder)"}[sig["kind"]]
    ...
    tname = "SPIKE HUNTER" if sig["kind"] in ("SPIKE", "HUNT") else ("SCALP" if tf in ("15m", "30m") else "SWING")
...
STRAT_NAME = {"FVG": "FVG Retest", "SPIKE": "Spike Reversal (Limit Ladder)", "HUNT": "Spike Hunter (Liquidity Limit)"}
...
def build_message(sym, tf, sig, info):
    ...
    L.append("")
    for i in range(3):
        L.append(f"✅{'Limit' if pend else 'Entry'} {i + 1} : {fp(e[i])}")
        L.append("")
    if pend:
...
def analyze_symbol(sym, tf):
    """لیستی از (sig, info, message, candle_time, ctx) برمی‌گرداند (ممکن است چند سیگنال روی یک کندل باشد)"""
    h1, h2 = MTF_MAP[tf]
    ...
    for off in range(int(CFG.scan_back) + 1):
        ...
        if now_ms - tc > CFG.max_age_min * 60_000:
            break
        t1, t2 = trend_at(H1, tc), trend_at(H2, tc)
        if (t1 or t2) and detect_all(P, t, t1, t2):         # پیش‌فیلتر سریع
            cand = (t, tc, t1, t2)
            break
    if cand is None:
        return []
    ...
    for sig in detect_all(P, t, t1, t2, extra):
        ...
        if tv:
            if ("BUY" if sd == 1 else "SELL") in tv:
                score += 1
            elif ("SELL" if sd == 1 else "BUY") in tv and tv.startswith("STRONG"):
                continue                          # تریدینگ‌ویو کاملاً مخالف است

        wr, n = winrate(by_group(sym_trades, grp))
        if n >= 8:
...
def scan_tf(tf, dry=False):
    ...
    def work(sym):
        try:
            return sym, analyze_symbol(sym, tf)
        except Exception as e:  # noqa
    ...
    for sym, res in results:
        for sig, info, msg, tt, ctx in res:
            ...
            cd = 16 if sig["kind"] == "HUNT" else CFG.cooldown_candles
            last = sent.get(k, 0)
    ...
    log.info("scan %s done: %d symbols, %d signals", tf, len(CFG.symbols), n_sent)
