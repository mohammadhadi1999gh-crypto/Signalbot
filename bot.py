...
class CFG:
    ...
    spike_on = os.getenv("SPIKE_HUNTER", "0") == "1"             # اسپایک‌یاب (بعد از اسپایک) - پیش‌فرض خاموش
    hunter_on = os.getenv("HUNTER_LIMITS", "0") == "1"           # سفارش‌های Limit شکارچی روی استخر نقدینگی - پیش‌فرض خاموش
    ...
    fvgtl_on = os.getenv("FVG_TL_STRATEGY", "0") == "1"     # ستاپ‌های FVG/ترندلاین (استراتژی‌های قبلی) - پیش‌فرض خاموش
...
def scan_tf(tf, dry=False):
    ...
    only_grid = CFG.grid_on and not any((CFG.fvgtl_on, CFG.spike_on, CFG.hunter_on))
    base_kinds = frozenset({"GRID"}) if only_grid else None
    with ThreadPoolExecutor(max_workers=int(CFG.workers)) as ex:
        results = list(ex.map(lambda s: work(s, kinds=base_kinds), CFG.symbols))
